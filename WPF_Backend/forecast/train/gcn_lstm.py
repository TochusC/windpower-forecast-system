...
def train(df):
    ...
    if os.path.exists(os.path.join(model_dir, 'gcn_lstm.pdparams')):
        gcn_lstm.set_state_dict(paddle.load(os.path.join(model_dir, 'GCN_LSTM.pdparams')))

    # 设置优化器
    scheduler = paddle.optimizer.lr.ReduceOnPlateau(learning_rate=learning_rate, factor=0.5, patience=3, verbose=True)
    opt = paddle.optimizer.Adam(learning_rate=scheduler, parameters=gcn_lstm.parameters())
    ...
    train_epochs_loss = []
    valid_epochs_loss = []
    gcn_lstm_early_stopping = EarlyStopping(patience=patience, verbose=True,
                                           ckp_save_path=os.path.join(model_dir, 'gcn_lstm.pdparams'))
    ...
    for epoch in tqdm(range(epoch_num)):
        ...
        # ==================early stopping======================
        gcn_lstm_early_stopping(np.average(valid_epoch_loss), model=gcn_lstm)
        if gcn_lstm_early_stopping.early_stop:
...
def start_train(data_path_list, model_id_now, isError=False):
    ...
    model_dir = os.path.join('model', str(turbine_id), "gcn_lstm", str(model_id))
    ...
    forecastModel.model_size = get_readable_file_size(os.path.join(model_dir, 'gcn_lstm.pdparams'))
    forecastModel.save()
