...
class TSDataset(paddle.io.Dataset):
    ...
    def __init__(self, data,
                 ...
                 labels=['YD15'],
                 input_len=24 * 4, pred_len=1, data_type='train',
                 train_ratio=0.8, val_ratio=0.1):
        super(TSDataset, self).__init__()
        self.ts_col = ts_col  # 时间戳列
        self.use_cols = use_cols  # 训练时使用的特征列
        self.known_cols = known_cols  # 训练时使用的已知未来特征列
    ...
    def transform(self, df):
        ...
        if self.scale:
            ...
            train_data_future = future_values[border1s[0]:border2s[0]]

            self.x_scaler.fit(train_data_x)
            self.y_scaler.fit(train_data_y)
            self.future_scaler.fit(train_data_future)

            x_data = self.x_scaler.transform(x_values)
            y_data = self.y_scaler.transform(y_values)
            future_data = self.future_scaler.transform(future_values)
            ...
            pickle.dump(self.future_scaler,
                        open(os.path.join(model_dir, 'future_scaler.pkl'), 'wb'))
        else:
            x_data = x_values
            y_data = y_values
        ...
        self.future_feature = paddle.to_tensor(future_data[border1:border2], dtype='float32')
    ...
    def __getitem__(self, index):
        ...
        # 增加对未来可见数据的获取
        future_x = self.future_feature[r_begin:r_end]
        return seq_x, seq_y, ts_x, ts_y, future_x
...
class Encoder(paddle.nn.Layer):
    def __init__(self, known_feature=5, feat_num=6, hidden_size=256, num_layers=3, dropout_rate=0.3, input_len=24 * 4):
...
class Decoder(paddle.nn.Layer):
    def __init__(self, known_feature=6, feat_num=5, hidden_size=512, num_layers=3, dropout_rate=0.2, input_len=24 * 4,
                 pred_len=24 * 4):
        ...
        self.proj = paddle.nn.Linear(in_features=feat_num + 1, out_features=hidden_size)
        self.relu1 = paddle.nn.ReLU()
    ...
    def forward(self, x, history_x):
        history_x = paddle.reshape(history_x, [history_x.shape[0], 1, -1])
        x = paddle.concat([x, history_x], axis=-1)
...
def train(df):
    ...
    if os.path.exists(os.path.join(model_dir, 'encoder.pdparams')):
        encoder.set_state_dict(paddle.load(os.path.join(model_dir, 'encoder.pdparams')))
        decoder.set_state_dict(paddle.load(os.path.join(model_dir, 'decoder.pdparams')))

    # 设置优化器
    scheduler = paddle.optimizer.lr.ReduceOnPlateau(learning_rate=learning_rate, factor=0.5, patience=3, verbose=True)
    opt = paddle.optimizer.Adam(learning_rate=scheduler, parameters=encoder.parameters() + decoder.parameters())
    ...
    train_epochs_loss = []
    valid_epochs_loss = []
    encoder_early_stopping = EarlyStopping(patience=patience, verbose=True,
                                           ckp_save_path=os.path.join(model_dir, 'encoder.pdparams'))
    decoder_early_stopping = EarlyStopping(patience=patience, verbose=True,
                                           ckp_save_path=os.path.join(model_dir, 'decoder.pdparams'))
    ...
    for epoch in tqdm(range(epoch_num)):
        ...
        for batch_id, data in enumerate(train_loader()):
            ...
            # 预测
            history = encoder(x)
            output = decoder(future_x, history)
            # 计算损失
        ...
        print('Valid:MSE of YD15:{}'.format(np.average(train_epoch_mse)))

        # ==================early stopping======================
        decoder_early_stopping(valid_epochs_loss[-1], model=decoder)
        encoder_early_stopping(valid_epochs_loss[-1], model=encoder)
    ...
    for batch_id, data in tqdm(enumerate(test_loader())):
        ...
        # 计算损失
        mse = mse_loss(output, y)
        acc = calc_acc(y.numpy().squeeze(0)[:, :], output.numpy().squeeze(0))
...
def get_readable_file_size(file_path):
    ...
    file_size1 = os.path.getsize(os.path.join(file_path, 'encoder.pdparams'))
    file_size2 = os.path.getsize(os.path.join(file_path, 'decoder.pdparams'))
...
def start_train(data_path_list, model_id_now, isError=False):
    ...
    model_dir = os.path.join('model', str(turbine_id), "gcn_lstm_mlp", str(model_id))
    ...
    forecastModel.model_size = get_readable_file_size(model_dir)
    forecastModel.save()
